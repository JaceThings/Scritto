...
Each edge earns a band by travelling, since a travelling edge is what leaves ink
behind. The band is worn by the layer holding the leaving glyphs, never by the
host, so the live value is never dimmed — and no ink wears two bands, which would
square the ramp into the hard edge the mask exists to avoid.
...
A mask paints no further than its element's border box, so that box is grown past
the ink on every side that is not being faded. [masking.md](masking.md) is the
whole subject: where the room comes from, the rules that keep a fade from
becoming a cut, and how each one was measured.
