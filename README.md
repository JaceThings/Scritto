...
**Every glyph renders in its own span, which is the price of the whole effect.** Nothing can slide independently unless it is its own box, so ligatures and kerning do not apply to a value while Scritto owns it, and a value breaks only between its words, never inside one. If you need a paragraph of text to reflow, this is the wrong tool; if you need a number, a price, a status or a short label to change without the eye losing it, this is exactly the tool.
...
**A value only breaks between its words.** Spaces inside a value become non-breaking and each word is its own inline-block box, so a line breaks between words and never inside one. A value with no spaces, a long formatted number for instance, has nowhere to break and runs past a narrow container rather than wrapping, so you size the container rather than the value.
